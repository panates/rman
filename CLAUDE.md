...
- **A target contributes its config keys too**, through the `PublishTargetConfigs` slot
  ...
  npm target declares `publish.directory`, each from its own package. A slot rather than a second
  declaration of `publish`, because a key arriving from two places does not compile - see
  "Config types". **Still open**: `publish.directory` should become `publish.npm.directory`, so
  every target's block is named after the target the way `docker`'s already is.
