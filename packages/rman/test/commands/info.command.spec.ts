...
describe('commands/info', () => {
  ...
  describe('plain (table-ish) output', () => {
    ...
    /**
     * A manifest with no name no longer prints `(none)`: `Manifest.read` falls back to the
     * *directory* name at version `0.0.0`, deliberately - the directory name is a fact, and `info`
     * has to work in a repository whose `.rmanrc` names no plugin at all. `(none)` was the old
     * answer to a question the fallback now answers better.
     */
    it("names a package after its directory when the manifest doesn't", async () => {
      const dir = tmp();
      writeJson(dir, 'package.json', {});

      const lines = await captureLogs(() => runCli({ cwd: dir, argv: ['info'] }));
      const joined = lines.join('\n');
      expect(joined).toContain(path.basename(dir));
      expect(joined).toContain('0.0.0');
    });
  });
