...
describe('commands/build', () => {
  ...
  it('is an alias for "run build" - no <script> positional needed', async () => {
    const dir = tmp();
    ...
    const lines = await captureLogs(() => runCli({ cwd: dir, argv: ['build', '--no-progress'] }));
    expect(lines.some(l => l.includes('build') && l.includes('pkg-a') && l.includes('success'))).toBe(true);
  });

  it('only ever runs "build", ignoring any other npm script the package defines', async () => {
    const dir = tmp();
    writeJson(dir, 'package.json', { name: 'root', private: true, workspaces: ['packages/*'] });
    /** Marks the repository root: `Workspace.findRoot` looks for an `.rmanrc*` or a `.git`,
     *  since it runs before the plugins that would know what a package is. */
    ...
    const lines = await captureLogs(() => runCli({ cwd: dir, argv: ['build', '--no-progress'] }));
    expect(lines.some(l => l.includes('echo built'))).toBe(true);
  ...
  it("shares run's own options, e.g. --from-root, through the same applyRunOptions() builder", async () => {
     ...
     *  since it runs before the plugins that would know what a package is. */
    fs.writeFileSync(path.join(dir, '.rmanrc'), '{}');
    writeJson(dir, 'packages/a/package.json', { name: 'pkg-a', version: '1.0.0', scripts: { build: quiet('echo a') } });
