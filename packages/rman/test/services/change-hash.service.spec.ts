...
describe('utils/detectChangeHash', () => {
  ...
  it('returns an explicit hash verbatim, without consulting npm at all', async () => {
    ...
    const hash = await ChangeHashService.detect(git, pkg, { from: 'deadbeef' });
    expect(hash).toBe('deadbeef');
    /** An explicit hash short-circuits before anything is read - not even the package's own
     *  ecosystem is asked. */
  ...
  it('auto-detects (default, no "from") the tag matching the published npm version', async () => {
    ...
    registryVersions.set(pkg.name, '1.2.3');
    const hash = await ChangeHashService.detect(git, pkg);
    expect(hash).toBe('v1.2.3');
  });

  it('"from: auto" behaves the same as omitting it', async () => {
    const dir = tmp();
    const pkg = makePackage(dir);
    initRepo(dir);
    execFileSync('git', ['tag', 'v1.2.3'], { cwd: dir });
    ...
    const hash = await ChangeHashService.detect(git, pkg, { from: ChangeHashService.AUTO });
    expect(hash).toBe('v1.2.3');
  });
  ...
  it('respects {name} in tagPattern for independent per-package versioning', async () => {
    ...
    registryVersions.set(pkg.name, '2.0.0');
    const hash = await ChangeHashService.detect(git, pkg);
  ...
  it('returns undefined when the published version has no matching tag in this repo', async () => {
    ...
    registryVersions.set(pkg.name, '9.9.9');
    const hash = await ChangeHashService.detect(git, pkg);
    expect(hash).toBeUndefined();
  });
  ...
  it("prefers this package's own latest tag over a stale npm-resolved one when both exist", async () => {
    ...
    /** A tag resolved the boundary, so the registry fallback is never reached. */
    expect(registryCalls).toEqual([]);
  });
  ...
  describe('catchUpFile (avoiding a documentation gap)', () => {
    it("widens the boundary to the file's own last-modifying commit when it is older than the npm-detected tag", async () => {
      ...
      const git = new GitHelper({ cwd: dir });
      registryVersions.set(pkg.name, '1.5.0');
      const hash = await ChangeHashService.detect(git, pkg, { catchUpFile: changelogFile });
    ...
    it('has no effect when the file does not exist', async () => {
      ...
      registryVersions.set(pkg.name, '1.5.0');
      const hash = await ChangeHashService.detect(git, pkg, { catchUpFile: path.join(dir, 'CHANGELOG.md') });
    ...
    it('is ignored when "from" is an explicit hash', async () => {
      ...
      const hash = await ChangeHashService.detect(git, pkg, { from: 'deadbeef', catchUpFile: changelogFile });
      expect(hash).toBe('deadbeef');
    });
