...
describe('augmentation/system-info', () => {
  ...
  it("reports rman's own version, and no longer a package that does not exist", async () => {
    /** It listed `rman-node` until the plugin was folded into rman - so every `rman info` printed
     *  `rman-node : Not Found`, which reads as a broken install rather than as a package that is
     *  simply gone. */
    await SystemInfo.getSystemInfo();
    expect(seen?.envinfo?.npmPackages).toEqual(['rman', 'typescript']);
  });
