...
describe('utils/version-stamp', () => {
  ...
  describe('stampVersionConstant()', () => {
    ...
    /**
     * `undefined` means **nothing matched**, not "no change was needed" - the two used to be the
     * same answer, and conflating them made the caller's error message a liar: `version.stamp`
     * reports a listed file that holds no version it can rewrite, and a file already sitting at the
     * target version is not that.
     */
    it('returns undefined only when nothing matched at all', () => {
      expect(stampVersionConstant('export const other = 42;', '6.0.10')).toBeUndefined();
    });

    it('returns the content unchanged when it matched but already holds this version', () => {
      const already = "export const version = '6.0.10';";
      expect(stampVersionConstant(already, '6.0.10')).toBe(already);
    });
