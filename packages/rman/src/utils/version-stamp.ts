...
/**
 * Rewrites the **quoted string assigned to `name`** (default `version`) to `version`, returning the
 * new content - or `undefined` when there was nothing to change, so a caller can write nothing and
 * report a file that matched nothing. Both an assignment and an object property are matched:
 *
 * ```ts
 ...
 *
 * **What it does not reach**, all measured, and the reason the decision is a seam: an identifier
 * with anything between it and the value (`pub const VERSION: &str = "…"`, and equally TypeScript's
 * own `const version: string = '…'`), an unquoted value (`pom.xml`'s `<version>1.0.0</version>`),
 * and - unless `name` says so - any other spelling (`VERSION`, `Version`, `__version__`,
 * `appVersion`). `name` was dead for exactly that reason: nothing passed it, so only the lowercase
 * word `version` was ever matched.
 *
...
export function stampVersionConstant(content: string, version: string, name = 'version'): string | undefined {
  ...
  const result = content.replace(pattern, (_whole, prefix: string, quote: string) => {
    matched = true;
    return `${prefix}${quote}${version}${quote}`;
  });
  /**
   * `undefined` means **nothing matched**, not "no change was needed" - the two used to be the same
   * answer, and that made the caller's error message a liar: a file already holding the target
   * version reported that nothing in it could be rewritten. A caller that wants to skip an identical
   * write compares the result with what it passed in.
   */
